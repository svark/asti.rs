...
impl <Point:VectorSpace> SplineWrapper for PeriodicBspline<Point>
{
    ...
    fn from_spline(spl: Bspline<Self::TW> ) -> Self { PeriodicBspline{ spl: spl } }
}
...
impl <Point:VectorSpace> PeriodicBspline<Point>
{
    ...
    pub fn new_from_unwrapped( pts: Vec<Point> , ks : Vec<f64>, degree : usize) -> Self
    {
        ...
        // let fm : Vec<f64> = end_diffs.iter().scan(f,|&mut &s,&x| { *s -= x; Some(*s)}).collect();
...
{
    type T = Point;
    fn param_range(&self) -> (f64,f64) { return self.spl.param_range() }
...
fn it_works() {
    use point::{Point2};
    let pts = vec!(Point2::new(0.0,0.0), Point2::new(0.4,0.3), Point2::new(0.2,0.8), Point2::new(-0.2,0.4) );
...
//   fixme: assert!(bs.is_valid().is_ok());
    assert_eq!(a, 0.0);
    assert_eq!(b, 1.0);
