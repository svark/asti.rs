...
pub fn find_param_2d_weighted(p: &Point2, bf: &Bezier<Point3>) -> Option<f64>
{
...
pub fn find_param_2d(p: &Point2, bf: &Bezier<Point2>) -> Option<f64>
{
    let lifted_pts = bf.control_points().iter().map(|&v| {v.hdim(1.0)}).collect::<Vec<_> >();
    find_param_2d_weighted(p, &Bezier::new(lifted_pts, bf.param_range().0, bf.param_range().1))
}
